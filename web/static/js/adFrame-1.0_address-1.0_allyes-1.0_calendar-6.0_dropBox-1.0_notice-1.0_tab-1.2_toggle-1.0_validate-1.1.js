;(function($){
	var cls={
 	...
 	function adFrame(obj,opt){
		this._init(obj,opt);
	}
	$.extend(adFrame.prototype,{
        _init:function(obj,opt){
            this._obj = obj;
            this._opt = opt;
            this._opts = $.extend(adFrame.defaults.setings,opt||{});
            var $ref=adFrame.defaults.$ref;
            var mod_adFrame_src = this._opts.mod_adframe_src;
            var mod_adFrame_style = this._opts.mod_adframe_style;
            obj.setAttribute("mod_adFrame_src",mod_adFrame_src);
            obj.setAttribute("mod_adFrame_style",mod_adFrame_style);
            $ref.list.push(obj);
            if (!$ref.clock){
                if ($ref.count)
                    frameAdInit();
            ...
            function frameAdInit(){
               var ad=$ref.list.shift();
                var src=ad.getAttribute("mod_adFrame_src");
...
;(function($){
	...
	function allyes(obj,opt){
		this._init(obj,opt);
	}
	
	$.extend(allyes.prototype,{
        _init:function(obj,opt){
            this._obj = obj;
            this._opt = opt;
            this._opts = $.extend(true,allyes.defaults.setings,opt||{});
           
            var attrs =allyes.defaults.attrs;
            var c_div_template =allyes.defaults.tmpls.c_div_template;
            var c_txt_template =allyes.defaults.tmpls.c_txt_template;
            var c_frm_template =allyes.defaults.tmpls.c_frm_template;
         
            var user =this._opts.user;
            var mod_allyes_user =this._opts.mod_allyes_user;
            ...
            var user=user||mod_allyes_user;
            if(!user){
                var buttons=attr(attrs[0],window);
                var text=attr(attrs[1],c_allyes_text);
    ...
    allyes.defaults = {
        setings:{
            "user":null,
            "mod_allyes_user":null
        },
        attrs:["mod_allyes_buttons","mod_allyes_text"],
        tmpls:{
           "c_div_template":'<div class="base_ad140x60" style="height:{$height}px">{$iframe}<\/div>',
           "c_txt_template":'<div class="base_adtxt140">{$text}<\/div>',
           "c_frm_template":'<iframe marginheight="0" width="100%" height="100%" marginwidth="0" frameborder="0" scrolling="no" src="http:\/\/allyes.ctrip.com\/main\/adfshow?user={$user}&db=ctrip&border=0&local=yes"><\/iframe>'
        }
    }
    $.mod.reg(cls);
})(cQuery);
 ...
 * @class switch(1.0)
 * toggle
 * @author ffpan@ctrip.com
 * @param {Object} opt  配置项
 * @return {Object}  notice的实例
*/
		...
		*/
		...
		_init: function(obj, opt){
			if (!opt.name||$.type(name)!='string'){
				cQuery.error('mod ('+cls.name+','+cls.version+') init','invalid name '+name);
				return;
			}
			
			this.target = $(obj);
			this.switchClass = opt.switchClass;
			this.switchClassMod = opt.switchClassMod;
			this.switchEl = opt.switchEl;
			...
			this._initToggleConfig();
			this.bindEvent();
		},
		
		_initToggleConfig: function(){
			var _this = this;
			this.subtarget = this.target[0].getElementsByTagName(_this.tagname);
			if(this.subtarget.length != this.switchEl.length){return false;}
		},
		
		_switchTab: function(e){
			var el = e.target;
			while( el.tagName != this.tagname ){
				el = el.parentNode;
				if(el == null){return false;}
			}
			//if(el.tagName != this.tagname){return false;}
			
			var di = el.getAttribute("data-index");
			this._setTab(di, el);
		},
		
		_setTab: function(di, el){
			var _this = this;
			
			var s_el = this.switchEl[di];
			var h_el = this.switchEl[Math.abs(di%2-1)];
			
			if(s_el!=null){$(s_el).css("display", "");}
			if(h_el!=null){$(h_el).css("display", "none");}
			
			$(el).addClass(_this.switchClass);
			$(this.subtarget[Math.abs(di%2-1)]).removeClass(_this.switchClass);
			if(this.callback){
				this.callback.call(window, di);
			}
		},
		...
		resetTab: function(index){
			var el = this.subtarget[index];
			this._setTab(index , el);
		}
